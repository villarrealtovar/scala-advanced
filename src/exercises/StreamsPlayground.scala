...
class Cons[+A](hd: A, tl: => MyStream[A]) extends MyStream[A] {
  ...
  /*
    ...
    ... mapped.tail
   */
  def map[B](f: A => B): MyStream[B] = new Cons(f(head), tail.map(f)) // preserve lazy eval!
  ...
  def take(n: Int): MyStream[A] = {
    ...
    else new Cons(head, tail.take(n-1)) // preserve lazy eval!
  }
}
...
object StreamsPlayground extends App {
  ...
  /*
    ...
    [ 2 3 eratosthenes applied to [ 5 7 9 11 ...] filter by n % 3 != 0
    [ 2 3 5 .... .... ...
   */
  // eratosthenes stive
